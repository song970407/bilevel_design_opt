...
if not os.path.exists('mpc_experiment'):
    os.makedirs('mpc_experiment')


def run_mpc(mpc_config, env_config, model_config, graph_config):
    ...
    graph_list = graph_config['graph_list']

    num_sensors = graph_list[0].nodes['state'].data['pos'].shape[0]
    num_heaters = graph_list[0].nodes['action'].data['pos'].shape[0]
    ...
    for i in range(num_targets):
        env = HeatDiffusionSystem(num_cells=num_cells,
                                  ...
                                  state_pos=state_pos_list[i],
                                  action_pos=action_pos_list[i])
        env.reset()
        env_list.append(env)
        num_states += state_pos_list[i].shape[0]
        num_actions += action_pos_list[i].shape[0]
        num_states_list.append(state_pos_list[i].shape[0])
        num_actions_list.append(action_pos_list[i].shape[0])
        if i < num_targets - 1:
            state_split_list.append(num_states)
            action_split_list.append(num_actions)

    graph = dgl.batch(graph_list).to(DEVICE)
    ...
    return np.split(trajectory_x, state_split_list, axis=1), np.split(trajectory_u, action_split_list, axis=1), trajectory_log
...
if __name__ == '__main__':
    ...
    for num_x in num_x_list:
        for num_heaters in num_heaters_list:
            if not os.path.exists('mpc_experiment/{}_{}'.format(num_x, num_heaters)):
                os.makedirs('mpc_experiment/{}_{}'.format(num_x, num_heaters))
            ...
            for model_name in model_names:
                for data_ratio in data_ratio_list:
                    ...
                    x_trajectory_list, u_trajectory_list, log_trajectory_list = run_mpc(mpc_config,
                                                                                        ...
                                                                                        model_config,
                                                                                        graph_config)
                    with open('mpc_experiment/{}_{}/mpc_config_{}_{}.pkl'.format(num_x, num_heaters, model_name, data_ratio),
                              'wb') as f:
                    ...
                    with open('mpc_experiment/{}_{}/mpc_experiment_result_{}_{}.pkl'.format(num_x, num_heaters, model_name, data_ratio),
                              'wb') as f:
